...
export class Chat implements OnInit, OnDestroy {
    ...
    private conversationRefreshInterval: any = null;
    private onlineRefreshInterval: any = null;
    private activeMessagesRefreshInterval: any = null;
    ...
    ngOnDestroy(): void {
        ...
        if (this.onlineRefreshInterval) clearInterval(this.onlineRefreshInterval);
        if (this.activeMessagesRefreshInterval) clearInterval(this.activeMessagesRefreshInterval);
    }
    ...
    selectConversation(conv: Conversation): void {
        ...
        this.restartActiveMessageRefresh();
    ...
    private refreshActiveMessagesQuietly(): void {
        const conv = this.activeConversation();
        if (!conv) return;
        this.chatService.getMessages(conv.conversationId, 0, 100).subscribe({
            next: (res) => {
                if (res.success && res.data) {
                    this.messages.set(res.data.content.reverse());
                    this.scrollToBottom();
    ...
    private restartActiveMessageRefresh(): void {
        if (this.activeMessagesRefreshInterval) {
            clearInterval(this.activeMessagesRefreshInterval);
        }
        this.activeMessagesRefreshInterval = setInterval(() => {
            this.refreshActiveMessagesQuietly();
        }, 3500);
    }
